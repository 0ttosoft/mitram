...
class Settings(BaseSettings):
    ...
    OLLAMA_REQUEST_TIMEOUT: float = Field(
        default=300.0, description="Request timeout for Ollama API in seconds"
    )
    OLLAMA_KEEP_ALIVE: str = Field(
        default="5m", description="Keep models loaded in memory (e.g., '5m', '1h')"
    )
    CHAT_MODEL: str = Field(
        default="gemma3:4b", description="Name of the chat model to use"
    )
    EMBEDDING_MODEL: str = Field(
        default="embeddinggemma:latest", description="Name of the embedding model to use"
    )
