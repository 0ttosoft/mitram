...
class RAGRepository:
    ...
    def _setup_models(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Models configured: LLM=%s, Embedding=%s (timeout=%ss, keep_alive=%s)",
                settings.CHAT_MODEL,
    ...
    def query(self, query_request: QueryRequest) -> QueryResponse:
        ...
        try:
            ...
            query_engine = self.index.as_query_engine(
                ...
                response_mode="compact",  # Faster than tree_summarize, single LLM call
    ...
    def warm_up_models(self) -> bool:
        """Warm up models by making test requests to load them into memory.
        
        This should be called at application startup to preload models.
        
        Returns:
            bool: True if warm-up was successful
        """
        try:
            ...
            try:
                test_embedding = Settings.embed_model.get_text_embedding(
                    "This is a test to warm up the embedding model."
                )
                logger.info(
                    "✓ Embedding model loaded successfully (dim=%s)", len(test_embedding)
                )
            except Exception as e:
            ...
            try:
                test_response = Settings.llm.complete(
                    "Say 'ready' if you can respond."
                )
                logger.info(
                    "✓ Chat model loaded successfully (response: %s)",
                    str(test_response)[:50],
                )
            except Exception as e:
                logger.error("✗ Failed to warm up chat model: %s", e)
                return False
