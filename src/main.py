...
async def lifespan(app: FastAPI):
    logger.info("🚀 Ultimate Advisor API server starting...")
    ...
    logger.info("🔥 Warming up Ollama models (this may take 30-60 seconds)...")
    try:
        success = rag_repository.warm_up_models()
        if success:
            logger.info("✅ Models are ready! Server is now accepting requests.")
        else:
            logger.warning("⚠️  Model warm-up failed. First requests may be slow.")
    except Exception as e:
        logger.error(f"❌ Error during model warm-up: {e}")
        logger.warning("⚠️  Continuing startup, but first requests may be slow.")
